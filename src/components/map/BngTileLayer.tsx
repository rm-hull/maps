...
class BngTileLayerClass extends L.TileLayer {
  ...
  getTileUrl(coords: L.Coords): string {
    ...
    const nwPoint = map.unproject(
      [coords.x * tileSize, coords.y * tileSize],
      coords.z
    );
    const sePoint = map.unproject(
      [(coords.x + 1) * tileSize, (coords.y + 1) * tileSize],
      coords.z
    );

    const centerPoint = L.latLng(
      (nwPoint.lat + sePoint.lat) / 2,
      (nwPoint.lng + sePoint.lng) / 2
    );

    // 2. Reproject the geographical coordinates (lat, lng) to British National Grid (BNG)
    const bngCoords = toBNG(centerPoint);

    const bngEasting = Math.round(bngCoords[0]);
    const bngNorthing = Math.round(bngCoords[1]);

    // 3. Construct the tile URL using the reprojected coordinates
    const bngTileUrl = this._url
      .replace('{z}', String(coords.z))
      .replace('{x}', String(bngEasting / 1000))
      .replace('{y}', String(bngNorthing / 1000));
